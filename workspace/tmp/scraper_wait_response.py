...
# -*- coding: utf-8 -*-
"""
网站数据抓取脚本 - 使用 wait_for_response 版本
"""
...
def click_and_wait_for_decrypt(page, cell):
    """点击解密按钮并等待API响应"""
    try:
        # 查找解密按钮
        ...
        if decrypt_btn.count() > 0 and decrypt_btn.is_visible(timeout=500):
            # 使用 wait_for_response 等待API响应
            with page.expect_response(
                lambda response: "queryCustomerByCopy" in response.url,
                timeout=5000
            ) as response_info:
                # 点击按钮
    ...
    try:
        ...
        for idx, row in enumerate(rows, 1):
            try:
                ...
                if masked_phone == "空":
                    print(f"   [{idx:2d}] ⚠️  无法提取脱敏手机号，跳过")
                    continue
                
                # 点击解密按钮并等待响应
                print(f"   [{idx:2d}] {parse_cell_text(cells[6]):10s}", end="", flush=True)
                decrypted_phone = click_and_wait_for_decrypt(page, phone_cell)
                
                # 如果解密成功，使用解密后的手机号；否则使用脱敏号码
...
def main():
    print("\n" + "="*60)
    print("🚀 网站数据抓取工具（wait_for_response版）")
    print("="*60)
